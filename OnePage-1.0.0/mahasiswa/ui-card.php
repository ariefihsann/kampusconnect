...
// Query gabungan datadosen dan tugas
$sql = "
  SELECT d.*, t.*, 
         CONCAT(d.hari, ' ', d.jam_mulai, '-', d.jam_selesai) AS jadwal, 
         d.ruangan AS ruang
  FROM datadosen d
  LEFT JOIN tugas t ON d.id_dosen = t.id_dosen
";
      ...
      <div class="container-fluid">
        <div class="card shadow-sm border-0">
          <div class="card-body">
            ...
            <div class="row g-4">
              <?php
              // Jangan cek $sql (itu string), cek $result (hasil query)
              if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                  $id_dosen = $row['id_dosen'];
                  $id_tugas = $row['id_tugas'] ?? null; // Use null coalescing in case id_tugas doesn't exist
                  $id_mahasiswa = $_SESSION['id_mahasiswa'] ?? null; // Ambil id_mahasiswa dari session jika ada
              ?>
                  <!-- Mata Kuliah Card -->
                  <div class="col-lg-4 col-md-6">
                    <div class="card h-100 course-card border-0 shadow-sm animate__animated animate__fadeInUp" data-animation-delay="100">
                      <div class="card-header bg-blue text-white d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0"><?= htmlspecialchars($row['mata_kuliah'] ?? 'Mata Kuliah') ?></h5>
                        <iconify-icon icon="fluent:math-formula-24-filled" class="fs-4"></iconify-icon>
                      </div>
                      <div class="card-body">
                        <div class="d-flex align-items-center mb-3">
                          <div>
                            <p class="mb-0 fw-medium"><?= htmlspecialchars($row['nama_dosen'] ?? 'Dosen') ?></p>
                        ...
                        <div class="d-flex align-items-center text-blue mb-2">
                          ...
                          <span><?= htmlspecialchars($row['jadwal'] ?? 'Jadwal') ?></span>
                        </div>
                        <div class="d-flex align-items-center text-muted">
                          <iconify-icon icon="solar:map-point-wave-bold-duotone" class="me-2"></iconify-icon>
                          <span><?= htmlspecialchars($row['ruang'] ?? 'Ruang') ?></span>
                        </div>
                        <?php if (!empty($row['judul_tugas'])): ?>
                          <div class="mt-2">
                            <small class="text-muted">Tugas: <?= htmlspecialchars($row['judul_tugas']) ?></small>
                          </div>
                        <?php endif; ?>
                      </div>
                      <div class="card-footer bg-light border-top-0 text-center py-3">
                        <a href="./datakuliah.php?id_dosen=<?= $id_dosen ?><?= $id_tugas ? '&id_tugas=' . $id_tugas : '' ?>&id_mahasiswa=<?= $id_mahasiswa ?>" class="btn btn-outline-blue btn-sm px-3">
              ...
              <?php
                ...
                echo '<div class="col-12"><p class="text-center">Tidak ada data ditemukan.</p></div>';
              }
              ?>
            </div>
          </div>
